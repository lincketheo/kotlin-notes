package exceptions.blogpost.example2
...
class CachingRepository(
    ...
    fun refresh() {
        retrofitClient.getData()?.let {
            localStorage.storeData(it)
        }
    }
}
...
class FakeNetworkDataSource {
    ...
    fun getData(): MyData? {
        return try {
            ...
            println("Http response timed out, cache is unchanged - probably show a pop up to the user. Exception: ${e.stackTrace}")
            null
        }
    }
